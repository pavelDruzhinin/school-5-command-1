    ...
    public static class Seed
    {
        ...
        public static async Task SeedBots(IServiceProvider services)
        {
           ...
           testchatbot = await FindChatByNameAndAuthorName(services, "test", "chatbot_test2");
            ...
            {
                var listque = new List<Question>();
                    listque.Add(new Question { Text = "Приветствие" , Value = "{ \"button\":\"Hello\" }" });
                    listque.Add(new Question { Text = "Какой Ваш возраст?", Value = "{\"group\":[{ \"button\":\"Меньше 18\"}, {\"button\":\"18-30\"},{\"button\":\"31-45\"}, {\"button\":\"Больше 45\"}]}" });
                    listque.Add(new Question { Text = "Чем Вы занимаетесь?", Value = "{\"group\":[{\"button\":\"Фронтенд\"}, {\"button\":\"Бэкенд\"},{\"button\":\"Ничем\"}]}" });
